...
TEST( bimap, multi_items )
{
    ...
    EXPECT_EQ( 10, map.get_key_by_value( "aba"  ) );
    EXPECT_EQ( 20, map.get_key_by_value( "caba" ) );
    EXPECT_EQ( 30, map.get_key_by_value( "xxyy" ) );
    EXPECT_FALSE( map.empty() );
    EXPECT_EQ( 3, map.size() );

    EXPECT_EQ( "aba" , map.get_value_by_key( 10 ) );
    EXPECT_EQ( "caba", map.get_value_by_key( 20 ) );
    EXPECT_EQ( "xxyy", map.get_value_by_key( 30 ) );
    EXPECT_FALSE( map.empty() );
    EXPECT_EQ( 3, map.size() );
...
TEST( bimap, insert_element_with_existing_value_or_key )
{
    ...
    EXPECT_EQ( "xxyy", map.get_value_by_key( 3 ) );

    EXPECT_EQ( 1, map.get_key_by_value("aba"  ) );
    EXPECT_EQ( 2, map.get_key_by_value("caba" ) );
    EXPECT_EQ( 3, map.get_key_by_value("xxyy" ) );
...
TEST( bimap, delete_existing_element )
{
    ...
    EXPECT_ANY_THROW( map.get_value_by_key( 20 ) );
    EXPECT_ANY_THROW( map.get_key_by_value( "caba" ) );
    
    EXPECT_TRUE( map.erase_by_value( "aba" ) );
    EXPECT_ANY_THROW( map.get_value_by_key( 10 ) );
...
TEST( bimap, copys_independence )
{
    ...
    EXPECT_ANY_THROW( b.get_key_by_value( "xxyy" ) );
...
TEST( bimap, iterate_over_collection )
{
    ...
    for( auto i = map.begin(); i != map.end(); i++ )
    {
        ...
        EXPECT_TRUE( map.get_value_by_key( i->first ) );
        EXPECT_TRUE( map.get_key_by_value( i->second ) );
